...
def iterable_generator_fille2trees(func):
    def call_function(path, correctiondic={}, rhaps=False, encoding="utf-8"):
        trees = []
        # print("这是一个装饰器！！！！ attention, run decorator!!!!")
            ...
            for li in f:
                ...
                if rhaps:
                    ...
                    if li:
                        ...
                        tree.id = id
                        tree.text = conlltext.encode("utf-8")
                        tree.li_num = li_index
...
@iterable_generator_fille2trees
...
def trees2conllFile(trees, outfile, columns="WEIGHT"):
    """
        ...
        for i in sorted(tree.keys()):
            ...
            if columns == "u":  # conllu format
                ...
                if govk:
                    ...
                    gk, gv = "_", "_"
                f.write("\t".join(
                    [str(i), node.get("t", "_"), node.get("lemma", ""), node.get("tag", "_"), node.get("tag2", "_"),
                     "|".join([a + ":" + v for a, v in node.get("features", {}).iteritems()]), gk, gv,
                     "|".join([str(g) + ":" + govs.get(g, "_") for g in govk[1:]]), "_"]) + "\n")
                ...
                if gov:
                    for govid, func in gov:
                        ...
                        # else:
                        if columns == "WEIGHT":
                            f.write("\t".join([str(i), node.get("t", "_"), node.get("lemma", ""), node.get("tag", "_"),
                                               node.get("tag2", "_"), "_", str(govid), func, node.get("nbrel", "_"), "_"]) + "\n")


                        elif columns == 10:
                            f.write("\t".join([str(i), node.get("t", "_"), node.get("lemma", ""), node.get("tag", "_"),
                                               node.get("tag2", "_"), "_", str(govid), func, node.get("nbrel", "_"),
                                               "_"]) + "\n")
                            ...
                            f.write("\t".join(
                                [str(i), node.get("t", "_"), lemma, lemma or node.get("t", "_"), node.get("tag", "_"),
                                 node.get("tag", "_"), node.get("morph", "_"), node.get("morph", "_"), str(govid),
                    ...
                    if columns == 10:
                        f.write("\t".join(
                            [str(i), node.get("t", "_"), node.get("lemma", ""), node.get("tag", "_"), node.get("tag2", "_"),
                             "_", str(govid), func, "_", "_"]) + "\n")

                    elif columns == 14:
                        lemma = node.get("lemma", "_")
                        if lemma == "_": lemma = node.get("t", "_")
                        f.write("\t".join(
                            [str(i), node.get("t", "_"), lemma, lemma, node.get("tag", "_"), node.get("tag", "_"),
                             node.get("morph", "_"), node.get("morph", "_"), str(govid), str(govid), func, func, "_",
                             "_"]) + "\n")
